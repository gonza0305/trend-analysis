...
class KeyWordsPerCountry(SuiteBase):

    def __init__(self, context_, spark_, args_):
        self.context = context_
        self.spark = spark_
        self.DATASOURCE_NAME = "raw_search_tags.keywords_per_country"
        self.SUITE_NAME = "suite_to_validate_key_words_per_country_on_raw_search_tags_data"
    ...
    def get_dataframe_to_validate(self):
        """
        Prepare the data to validate, it returns a dataframe with the total of keywords per country.
        :return: pandas dataframe
        """
        current_raw_search_tags = self.spark.read.parquet(self.current_raw_search_tags_path)
        current_key_words_per_country_df = current_raw_search_tags\
            .select(current_raw_search_tags.do_id, current_raw_search_tags.country_code, explode(current_raw_search_tags.keyword).alias("keyword"))\
        ...
        before_key_words_per_country_df = before_raw_search_tags\
            .select(before_raw_search_tags.do_id, before_raw_search_tags.country_code, explode(before_raw_search_tags.keyword).alias("keyword"))\
            .groupBy("country_code").count()\
            .withColumnRenamed("count", "count_before")

        return before_key_words_per_country_df \
            .join(current_key_words_per_country_df, on=["country_code"], how="left") \
            ...
            meta={
                "notes": {
                    ...
                    "content": "Validate the percent of the difference between the previous and the current " +
                               "execution, checking the amount of keywords per country. \n" +
